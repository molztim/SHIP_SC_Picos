...
def changeCH(k,v,ch=8):
    pos = CONFIG_KEY_LIST[28:].index(k)
    old_value = new_config[28+(ch-1)*11+pos]
    new_config[28+(ch-1)*11+pos] = v
    
    print(f"Set '{k} Ch {ch}' {CONFIG_KEY_LIST.index(k)} {old_value} => {v} ")
...
VBG_ADJ = 2 # Lower Bound: 4
DVTH = 0#Lower Bound: 65
...
changeCH("DVOFFSET",141,ch=1)
changeCH("DVOFFSET",210,ch=2)
changeCH("DVOFFSET",265,ch=3)
changeCH("DVOFFSET",231,ch=4)
changeCH("DVOFFSET",180,ch=5)
changeCH("DVOFFSET",211,ch=6)
changeCH("DVOFFSET",210,ch=7)
changeCH("DVOFFSET",200,ch=8)
