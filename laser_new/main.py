...
IP = ('10.42.0.143','255.255.255.0', '10.42.0.1', '10.42.0.1')
...
fast_counter = 0
fast_limit = int(60/3 * 1) #last number in minutes, 2 due to readout time intervall of 2s
modecontrol = 1
    ...
    while True:
        try:
            ...
            if "INTERNAL" in response:
                ...
                if "DIST" in rcv:
                    calib = int(answer[1]) - int(ceil(float(answer[2])*1000))
                    log(f"Calibration Distance: {calib}")
                elif "STAT" in rcv:
                    laserstatus = int(answer[-1])
