# Version 4.0 For TB Fall 2025
# Now improves F+ and has multi byte settings
...
class device:
    ...
    def __init__(self,rtc):
        try:
            ...
            self.data_lock = _thread.allocate_lock() # The improved version
        ...
        if "GET_EMUSIC_CONF" in received:
            ...
            with self.data_lock:
                ...
                # Functions to receive, decode and set configurations
                    ...
                    decoded = binascii.a2b_base64(raw)
                    config = []
                    index = 0
                    ...
                    for key in alpha_keylist:
                        lenght_register = 1
                        if (any(isinstance(i, list) for i in fastic_registers[key])):
                            lenght_register = len(fastic_registers[key])
                        size_list.append(lenght_register)          
                    
                    for size in size_list:
                        chunk = decoded[index:index + size]
                        config.append(bytes_to_int(chunk))
                        index = index + size
                    #config = [int(raw[i:i+2],16) for i in range(0, len(raw), 2)]
                    
                    log("Config rcv.! #",self.configID,":",config[:5],"...",len(config))
                    with self.file_lock:
